...
export function loadStats() {
    ...
    fetch(url)
        ...
        .then(data => {
            revenueContainer.innerHTML = renderStat(formatCurrency(data.revenue.amount), data.revenue.percent);
            ordersContainer.innerHTML = renderStat(formatNumber(data.orders.amount), data.orders.percent);
            customersContainer.innerHTML = renderStat(formatNumber(data.customers.amount), data.customers.percent);
        });
}

function renderStat(amount, percent) {
    return `
        <h3 class="mt-3 mb-3"> ${amount}</h3>
