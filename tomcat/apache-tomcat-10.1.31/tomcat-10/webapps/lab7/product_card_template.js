...
$(document).ready(function () {
    const linkCSS = "product_card.css";
    const linkHTML = "product_card.html";
    importCSS(linkCSS);

    const $card_templates = $('.product_card_template');
    $card_templates.each(function () {
        const $card_template = $(this);
        $card_template.load(linkHTML, function () {
            ...
            if (data["basePrice"] && data["basePrice"] > 0) {
                $card_template.find('.product_base_price').text(100 || null);
            } else {
                $card_template.find('.product_base_price').remove();
            }
            // product_discount
            if (data["discount"]) {
